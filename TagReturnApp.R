...
1. Si el pez fue liberado, envíe una foto del pez con la etiqueta externa visible, junto con la información de la encuesta a continuación a fishtags@iastate.edu. No elimine la etiqueta externa.
2. Si el pescado fue cosechado, coloque un sobre con la etiqueta interna y la encuesta completa en un buzón ubicado en la estación de limpieza de pescado y la oficina del parque.')})
  output$selected_var3 <- renderText({if_else(input$Lang == 'English', paste(' <b> Attention Anglers'), paste(' <b> Atención pescadores:'))})
  ...
  output$selected_var6 <- renderText({if_else(input$Lang == 'English', paste(' Email tag reports, pictures or questions to: <b> fishtags@iastate.edu </b>, or call <b> (515) 294 - 7991 </b>'), paste(' Envíe por correo electrónico informes o preguntas sobre etiquetas a: fishtags@iastate.edu'))})
  output$selected_var8 <- renderText({if_else(input$Lang == 'English', paste(''), paste(' Envíe por correo electrónico informes o preguntas sobre etiquetas a: fishtags@iastate.edu'))})
  output$selected_var6 <- renderText({if_else(input$Lang == 'English', paste(' Email tag reports, pictures or questions to: <b> fishtags@iastate.edu </b>, or call <b> (515) 294 - 7991 </b>'), paste(' Envíe por correo electrónico informes o preguntas sobre etiquetas a: fishtags@iastate.edu'))})
